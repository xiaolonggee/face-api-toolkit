<template>
  <div id="app" v-cloak>
    ...
    <div class="btn-operate">
      <van-button @click="startMediaRecorder" v-if="isCameraOpen && !inProgress" type="info" native-type="button"
        >开始录制
      </van-button>
      <van-button v-if="isCameraOpen && inProgress" type="danger" native-type="button" disabled>录制中</van-button>
      <van-button @click="neuAufnehmen" v-if="!isCameraOpen && !inProgress" type="default" native-type="button"
        >重新录制
      </van-button>
    </div>
    ...
    <!-- 镜像 -->
    <div
      class="FlipHorizontal-toggle"
      :class="{ FlipHorizontalIng: flipHorizontal }"
      title="左右镜像"
      ...
      return new Promise(async resolve => {
        ...
        try {
          ...
          const detectorConfig = {
            ...
            runtime: 'mediapipe',
            solutionPath: './face_mesh' //WASM二进制文件和模型文件所在的路径
          }
